...
$ helm install my-release stable/<chart>           # Helm 3
$ helm install --name my-release stable/<chart>    # Helm 2
```
...
```console
helm install postgres-debezium --set extendedConfConfigMap=postgresql-config --set shmVolume.chmod.enabled=false --set securityContext.enabled=false --set image.tag=latest --set volumePermissions.securityContext.runAsUser=auto --set service.type=ClusterIP --set service.port=5432 --set postgresqlPassword=passw0rd stable/postgresql

or via value file

helm install postgres-debezium -f .\values.yaml stable/postgresql
```
